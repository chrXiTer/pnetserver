# -*- coding: utf-8 -*-
"""
    修改自 Flask-JWT module
    将 _default_***_handler 改为 _default_***_callback 便于代码阅读
"""
...
def _default_jwt_headers_callback(identity):
    return None


def _default_jwt_payload_callback(identity):
    iat = datetime.utcnow()
...
def _default_jwt_encode_callback(identity):
...
def _default_jwt_decode_callback(token):
    secret = current_app.config['JWT_SECRET_KEY']
    algorithm = current_app.config['JWT_ALGORITHM']
...
def _default_auth_request_callback():
...
def _default_auth_response_callback(access_token, identity):
    return jsonify({'access_token': access_token.decode('utf-8')})


def _default_jwt_error_callback(error):
    logger.error(error)
...
class JWT(object):
    ...
    def __init__(self, app=None, authentication_handler=None, identity_handler=None):
        ...
        self.auth_response_callback = _default_auth_response_callback
        self.auth_request_callback = _default_auth_request_callback
        self.jwt_encode_callback = _default_jwt_encode_callback
        self.jwt_decode_callback = _default_jwt_decode_callback
        self.jwt_headers_callback = _default_jwt_headers_callback
        self.jwt_payload_callback = _default_jwt_payload_callback
        self.jwt_error_callback = _default_jwt_error_callback
        self.request_callback = _default_request_callback
    ...
    def init_app(self, app):
        ...
        if auth_url_rule:
            if self.auth_request_callback == _default_auth_request_callback:
